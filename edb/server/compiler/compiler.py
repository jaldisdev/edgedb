...
class Compiler:
    ...
    def compile_sql(
        ...
        def translate_query(
            stmt: pgast.Base
        ) -> Tuple[pg_codegen.SQLSource, Optional[str]]:
            args = {}
            try:
            ...
            resolved, dml_tag = pg_resolver.resolve(
                stmt, schema, options
            )
            return (
                pg_codegen.generate(
        ...
        for stmt in stmts:
            ...
            if isinstance(stmt, pgast.VariableSetStmt):
                ...
                stmt_source, stmt_dml_tag = translate_query(stmt.query)
                if stmt.argtypes:
                    param_types = []
                ...
                unit = unit_ctor(
                    ...
                    command_complete_tag=stmt_dml_tag,
                )
                ...
                source, stmt_dml_tag = translate_query(stmt)
                unit = unit_ctor(
                    query=source.text,
