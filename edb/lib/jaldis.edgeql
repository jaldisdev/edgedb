...
ext::jaldis::generate_content_id(content_type: std::int64) -> std::uuid
{
    ...
    USING EDGEQL $$
        with unix_ts_ns := ((<int64>datetime_get(datetime_of_statement(), 'epochseconds') * 1000000000) + <int64>str_split(str_split(<str>datetime_of_statement(), '.')[1], '+')[0] * 1000),
            ...
            uuid_bytes := to_bytes(<int32>t1, Endian.Big) ++ to_bytes(<int32>t2, Endian.Big)[2:4] ++ to_bytes(<int16>t3 + <int16>bit_lshift(<int16>8, 12), Endian.Big) ++ to_bytes(<int16>bit_lshift(<int16>2, 14), Endian.Big) ++ to_bytes(<int16>bit_lshift(<int16>content_type, 2), Endian.Big) ++ to_bytes(uuid_generate_v4())[11:15]
        select to_uuid(uuid_bytes)
    $$;
};
